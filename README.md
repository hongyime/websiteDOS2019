# ddos-website-code

# ⚠️ ILLEGAL WITHOUT AUTHORIZATION - EDUCATIONAL PURPOSES ONLY ⚠️

**THIS IS A DENIAL OF SERVICE (DoS) ATTACK TOOL**

## 🚨 CRITICAL LEGAL WARNING

**USING THIS TOOL AGAINST SYSTEMS YOU DON'T OWN IS A FEDERAL CRIME**

### Legal Consequences

**United States:**
- **Computer Fraud and Abuse Act (CFAA)** - 18 U.S.C. § 1030
  - Up to 10 years imprisonment for first offense
  - Up to 20 years for repeat offenses
  - Fines up to $250,000

**International:**
- **UK Computer Misuse Act 1990** - Up to 10 years imprisonment
- **EU Directive 2013/40/EU** - Criminal penalties across EU
- **Similar laws exist in virtually every country**

### Civil Liability
- Victims can sue for damages
- Costs can reach millions of dollars
- Permanent criminal record

---

## ⚠️ Authorized Use Only

**Legal Uses:**
1. **Penetration Testing** - With explicit written authorization
2. **Security Research** - In isolated lab environments only
3. **Educational Study** - Understanding attack vectors (not execution)
4. **Your Own Systems** - Testing your own infrastructure

**Illegal Uses:**
- Attacking websites you don't own
- Attacking without written authorization
- "Testing" public websites
- Revenge attacks
- Competitive attacks
- Any unauthorized use

---

code to launch a ddos attack on an IP.

<p align="left">
  <img src="https://cybernews.com/wp-content/uploads/2019/01/What-is-a-DDoS-attack-750x375.png" width="550" height="300" />
</p>

## Disclaimer:

**⚠️ FOR EDUCATIONAL PURPOSES ONLY ⚠️**

1. **ILLEGAL WITHOUT AUTHORIZATION** - Using this tool against systems you don't own is a federal crime
2. **CRIMINAL PENALTIES** - Can result in imprisonment and massive fines
3. **CIVIL LIABILITY** - Victims can sue for damages
4. **PERMANENT RECORD** - Criminal conviction will follow you for life
5. **USE AT YOUR OWN RISK** - Author is not responsible for any misuse

**This tool demonstrates DoS attack techniques for educational purposes only.**

**DO NOT:**
- Attack websites or systems you don't own
- Use without explicit written authorization
- "Test" public websites
- Use for revenge, competition, or malicious purposes

**Violates:**
- Computer Fraud and Abuse Act (CFAA) - USA
- Computer Misuse Act - UK
- Similar laws in virtually every country worldwide

**Always obtain explicit written authorization before testing any system.**

---

## Instructions:

**⚠️ WARNING: Only use in authorized lab environments**

1. Download the repo as a zip file
2. Unzip the file
3. Open the python file in an editor of your choice
4. Paste in the IP which you want to attack and the Ip you want to spoof
5. Save and close the python file
6. Double click to run the programme

**REMINDER: Unauthorized use is ILLEGAL and will result in criminal prosecution.**

## License

Apache-2.0. See [LICENSE](LICENSE) and [NOTICE](NOTICE).
